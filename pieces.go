...
var BBSquares = [...]uint64{
	BBA1, BBB1, BBC1, BBD1, BBE1, BBF1, BBG1, BBH1,
	BBA2, BBB2, BBC2, BBD2, BBE2, BBF2, BBG2, BBH2,
	BBA3, BBB3, BBC3, BBD3, BBE3, BBF3, BBG3, BBH3,
	BBA4, BBB4, BBC4, BBD4, BBE4, BBF4, BBG4, BBH4,
	BBA5, BBB5, BBC5, BBD5, BBE5, BBF5, BBG5, BBH5,
	BBA6, BBB6, BBC6, BBD6, BBE6, BBF6, BBG6, BBH6,
	BBA7, BBB7, BBC7, BBD7, BBE7, BBF7, BBG7, BBH7,
...
func init() {
	for square, mask := range BBSquares {
		if (fileIndex(square) + rankIndex(square)%2) == 1 {
			BBLightSquares |= mask
		} else {
			BBDarkSquares |= mask
		}
	...
	for _, square := range Squares {
		BBSquaresL90[square] = BBSquares[SquaresL90[square]]
	}
	for _, square := range Squares {
		BBSquaresL45[square] = BBSquares[SquaresL45[square]]
	}
	for _, square := range Squares {
		BBSquaresR45[square] = BBSquares[SquaresR45[square]]
	}

	for _, bbSquare := range BBSquares {
		mask := BBVoid
		mask |= shiftLeft(shift2Up(bbSquare))
		mask |= shiftRight(shift2Up(bbSquare))
		mask |= shiftLeft(shift2Down(bbSquare))
		mask |= shiftRight(shift2Down(bbSquare))
		mask |= shift2Left(shiftUp(bbSquare))
		mask |= shift2Right(shiftUp(bbSquare))
		mask |= shift2Left(shiftDown(bbSquare))
		mask |= shift2Right(shiftDown(bbSquare))
		BBKnightAttacks = append(BBKnightAttacks, mask&BBAll)

		mask = BBVoid
		mask |= shiftLeft(bbSquare)
		mask |= shiftRight(bbSquare)
		mask |= shiftUp(bbSquare)
		mask |= shiftDown(bbSquare)
		mask |= shiftUpLeft(bbSquare)
		mask |= shiftUpRight(bbSquare)
		mask |= shiftDownLeft(bbSquare)
		mask |= shiftDownRight(bbSquare)
		BBKingAttacks = append(BBKingAttacks, mask&BBAll)
	}

	for _, square := range Squares {
		for bitrow := 0; bitrow < 64; bitrow++ {
			f := fileIndex(square) + 1
			q := square + 1
			for f < 8 {
				BBRankAttacks[square][bitrow] |= BBSquares[q]
				if (1<<uint(f))&(bitrow<<1) > 0 {
					break
				}
				q++
				f++
			...
			r := rankIndex(square) + 1
			q = square + 8
			for r < 8 {
				BBFileAttacks[square][bitrow] |= BBSquares[q]
				if (1<<uint(7-r))&(bitrow<<1) > 0 {
					break
				}
				q += 8
				r++
			...
			q = square
			for fileIndex(q) > 0 && rankIndex(q) < 7 {
				q += 7
				mask |= BBSquares[q]
				if uint64(bitrow)&(BBSquaresL45[q]>>BBShiftL45[square]) > 0 {
					break
				}
			}

			q = square
			for fileIndex(q) < 7 && rankIndex(q) > 0 {
				q -= 7
				mask |= BBSquares[q]
				if uint64(bitrow)&(BBSquaresL45[q]>>BBShiftL45[square]) > 0 {
					break
			...
			BBL45Attacks[square][bitrow] = mask

			mask = BBVoid

			q = square
			for fileIndex(q) < 7 && rankIndex(q) < 7 {
				q += 9
				mask |= BBSquares[q]
				if uint64(bitrow)&(BBSquaresR45[q]>>BBShiftR45[square]) > 0 {
					break
				}
			}

			q = square
			for fileIndex(q) > 0 && rankIndex(q) > 0 {
				q -= 9
				mask |= BBSquares[q]
				if uint64(bitrow)&(BBSquaresR45[q]>>BBShiftR45[square]) > 0 {
					break
			...
			BBR45Attacks[square][bitrow] = mask
		}
	}

	for i, s := range BBSquares {
		BBPawnAttacks[0][i] = shiftUpLeft(s) | shiftUpRight(s)
		BBPawnAttacks[1][i] = shiftDownLeft(s) | shiftDownRight(s)

		BBPawnF1[0][i] = shiftUp(s)
		BBPawnF1[1][i] = shiftDown(s)

		BBPawnF2[0][i] = shift2Up(s)
		BBPawnF2[1][i] = shift2Down(s)
	}

	for i := range Squares {
		BBPawnAll[0][i] = BBPawnAttacks[0][i] | BBPawnF1[0][i] | BBPawnF2[0][i]
		BBPawnAll[1][i] = BBPawnAttacks[1][i] | BBPawnF1[1][i] | BBPawnF2[1][i]
	}
}
...
func shift2Down(b uint64) uint64 {
	return b >> 16
}
...
func shift2Right(b uint64) uint64 {
	return (b << 2) & ^BBFileA & ^BBFileB
}
...
func shift2Left(b uint64) uint64 {
	return (b >> 2) & ^BBFileG & ^BBFileH
}
...
func l90(b uint64) uint64 {
	mask := BBVoid

	square := bitScan(b, 0)
	for square != -1 {
		mask |= BBSquaresL90[square]
		square = bitScan(b, square+1)
	}

	return mask
}

func r45(b uint64) uint64 {
	mask := BBVoid

	square := bitScan(b, 0)
	for square != -1 {
		mask |= BBSquaresR45[square]
		square = bitScan(b, square+1)
	}
...
func l45(b uint64) uint64 {
	mask := BBVoid

	square := bitScan(b, 0)
	for square != -1 {
		mask |= BBSquaresL45[square]
		square = bitScan(b, square+1)
	}
...
var PolyglotRandomArray = []uint64{
